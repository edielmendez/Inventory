...
app.controller('quejaController', function($scope, $http){
	$scope.getAll = function(){
		
		$http.get("../methods/read_quejas.php").success(function(response){
			
			$scope.quejas = response.records;
			console.log($scope.quejas);
		});
	}

	$scope.deleteQueja = function(id){
		$scope.id = id;
		$("#modal_eliminar_queja").openModal();
	     /*
	    // ask the user if he is sure to delete the record
	    if(confirm("Are you sure?")){
	        // post the id of product to be deleted
	...
	$scope.aceptDeleteQueja = function(){
		$http.post('../methods/delete_queja.php', {
	            'id' : $scope.id
	        }).success(function (data, status, headers, config){
	            console.log(data)
		        $('#modal_eliminar_queja').closeModal();
